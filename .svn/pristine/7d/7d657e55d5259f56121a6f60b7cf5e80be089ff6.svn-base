...
function pages($total, $limit, $rule = "") {
	...
	$config['full_tag_open'] = "<div class='fy'><div class='active'>";
 ...
 * @return boolean
 */
function maxMp3FileSize($file, $size = 10485760) {
	if ($file['size'] > $size) {
		return false;
	}
	return true;
...
function uploadMp3($file, $folder = "/tmp") {
		if (empty($file) || $file['error'] > 0) {
		return false;
	}
	$ext = get_extension($file['name']);
...
function status_items_status($status){
    $status = (int)$status;
    if($status>=0){
        switch ($status){
            case 0:
...
function menu_option($menu,$number,$role = null){
    $roleArr = array();
    if(!empty($role)){
    ...
    if(!empty($menu)){
        ...
        foreach($menu as $value){
            if(in_array($value['menu_id'],$roleArr)){
                echo "<li class='menu_li' value='".$value['menu_id']."'>".$strArr."<input name='group_role[]' class='show_role show_role_".$value['menu_id']."' type='checkbox' value='".$value['menu_id']."' checked='checked' />".$value['menu_name']."</li>";
            }else{
                echo "<li class='menu_li' value='".$value['menu_id']."'>".$strArr."<input name='group_role[]' class='show_role show_role_".$value['menu_id']."' type='checkbox' value='".$value['menu_id']."' />".$value['menu_name']."</li>";
            }
            if(!empty($value['children'])){
                 menu_option($value['children'],$number,$role);
            }
        }
        echo "</ul>";
    }
...
function telephoneCheck($str){
    $str = trim($str);
    $rule = '/^(0\d{2,3}-)\d{7,8}|^(0\d{2,3})( ?)\d[1-9]\d{6,7}|^\d{7,8}$|(1)([3,5,8]{1})(\d{9})$/A';
