...
class Login_service extends MY_Service {
	...
	public function __construct() {
		...
		$this->loadModel("MyUser","MySysLog");
	}
	...
	public function logindo($input) {
		...
		if ($cookiename == self::NORMAL_USER || $cookiename == self::SMARTOA_USER || $cookiename == self::NORMAL_USER_OTHER) {
			...
			$user = $this->MyUser_model->where(array(
				"user_name" => $user_name,
				"status" => MyUser_model::STATUS_YES
			))->get_item();
		}
		if (empty($user) || ($cookiename == self::SMARTOA_USER && $user['oauser'] == MyUser_model::OAUSER_NO) || (!$cookiename == self::SMARTOA_USER && $user['oauser'] == MyUser_model::OAUSER_YES)) {
		...
		if ($cookiename == self::SMARTOA_USER || $cookiename == self::NORMAL_USER_OTHER) {
			...
			if ($cookiename == self::NORMAL_USER) {
				...
				$this->MyUser_model->where("user_id", $user['user_id'])->update(array(
					"login_ip" => ip(),
					"login_time" => time()
				));
				$this->MySysLog_model->set(array(
