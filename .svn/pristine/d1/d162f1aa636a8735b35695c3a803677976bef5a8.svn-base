...
class MyMenu_service extends MY_Service {
	...
	public function __construct() {
		parent::__construct();
		$this->loadModel("MyMenuUrl","MySysLog");
	...
	public function addoredit($options = array()) {
		...
		if (optionExists($menu_hide)) {
			...
			$data['menu_hide'] = MyMenuUrl_model::HIDE_NO;
		}

		if ($father_menu > 0) {
			$m = $this->MyMenuUrl_model->where(array(
		...
		if (optionExists($menu_id)) {
			...
			$success = $this->MyMenuUrl_model->where("menu_id", $menu_id)->set($data)->update();
		} else {
			$success = $this->MyMenuUrl_model->set($data)->insert();
		}
	...
	public function editable($id) {
		$m = $this->MyMenuUrl_model->where(array("menu_id" => $id))->select("menu_built")->get_item();
		if (empty($m)) {
			return false;
		}
		if ($m['menu_built'] == MyMenuUrl_model::BUILT_YES) {
			return false;
		}
		return true;
	}
	...
	public function get_list($options = array()) {
		...
		$menu_url = $this->MyMenuUrl_model->where($where)->get_list();
	...
	public function delete($options = array()) {
		...
		$success = $this->MyMenuUrl_model->where("menu_id", $menu_id)->delete();
		if (!$success) {
			return wrrong_msg(MSG_SERVICE_BUSY);
		}
        $this->MySysLog_model->set(array(
