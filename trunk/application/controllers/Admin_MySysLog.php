...
class Admin_MySysLog extends MY_Controller {
	...
	public function index() {
        ...
        if (!empty($input)) {
            ...
            $log['rows'] = $this->MySysLog_model->where($where)->limit($rows,$offset)->order_by('op_time','desc')->get_list();
            $log['page'] = $input['page'];
            $log['count'] = $this->MySysLog_model->where($where)->count();
            $log['page_all'] = ceil($log['count']/$rows);
        }else{
            $log['rows'] = $this->MySysLog_model->limit($rows,0)->order_by('op_time','desc')->get_list();
            $log['page'] = 0;
            $log['count'] = $this->MySysLog_model->count();
            $log['page_all'] = ceil($log['count']/$rows);
        }
		$this->assign("log", $log);
