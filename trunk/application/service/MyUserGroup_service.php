...
class MyUserGroup_service extends MY_Service {
	...
	public function addoredit($options = array()) {
		...
		if ($group_id) {
			...
			$success = $this->MyUserGroup_model->where("group_id", $group_id)->set($data)->update();
		} else {
			if (empty($group_name)) {
				return wrrong_msg(self::MSG_USERNAME_NOT_NULL);
			}
            $usergroup = $this->MyUserGroup_model->where("group_name",$group_name)->get_list();
            if($usergroup){
                return wrrong_msg(self::MSG_USERNAME_IS_EXISTS);
            }
			$success = $this->MyUserGroup_model->set($data)->insert();
		}
	...
	public function editable($id) {
        if ($id == MyUserGroup_model::BUILT_ID) {
			return false;
		}
		return true;
	}
	...
	public function get_list($options = array()) {
		...
		if (optionExists($rows) && optionExists($page)) {
			$result['total'] = $this->MyUserGroup_model->where($where)->count();
			$page = max($page, 1);
			$offset = ($page - 1) * $rows;
			$this->MyUserGroup_model->limit($rows, $offset);
		}
		//获取列表
		$list = $this->MyUserGroup_model->where($where)->get_list();
		//获取用户组
		foreach ($list as $k => $v) {
			$user = $this->MyUser_model->where("user_id",$v['owner_id'])->get_item();
			if (!empty($user)) {
	...
	public function delete($options = array()) {
		...
		$success = $this->MyUserGroup_model->where("group_id", $group_id)->delete();
		if (!$success) {
			return wrrong_msg(MSG_SERVICE_BUSY);
		}
        $this->MySysLog_model->set(array(
